...
contract TaxOracle is Ownable {
    ...
    constructor(
        ...
        require(_egg != address(0), "egg address cannot be 0");
    ...
    function consult(address _token, uint256 _amountIn) external view returns (uint144 amountOut) {
        require(_token == address(egg), "token needs to be egg");
        uint256 eggBalance = egg.balanceOf(pair);
        uint256 wbnbBalance = wbnb.balanceOf(pair);
        return uint144(eggBalance.mul(_amountIn).div(wbnbBalance));
    }

    function getEggBalance() external view returns (uint256) {
	return egg.balanceOf(pair);
    }
    ...
    function getPrice() external view returns (uint256) {
        uint256 eggBalance = egg.balanceOf(pair);
        uint256 wbnbBalance = wbnb.balanceOf(pair);
        return eggBalance.mul(1e18).div(wbnbBalance);
    }


    function setEgg(address _egg) external onlyOwner {
        require(_egg != address(0), "egg address cannot be 0");
        egg = IERC20(_egg);
    }
