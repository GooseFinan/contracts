...
contract Egg is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the EGG ERC-20 contract.
     */
    constructor() public ERC20("Goose Finance", "EGG") {
        // Mints 1 EGG to contract creator for initial pool setup
        _mint(msg.sender, 1 ether);
    }

    function _getEggPrice() internal view returns (uint256 _eggPrice) {
        try IOracle(eggOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Egg: failed to fetch EGG price from Oracle");
        }
    }

    function setEggOracle(address _eggOracle) public onlyOperator {
        require(_eggOracle != address(0), "oracle address cannot be 0 address");
        eggOracle = _eggOracle;
    }

    /**
     * @notice Operator mints EGG to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of EGG to mint to
